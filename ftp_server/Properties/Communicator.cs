 ...
 */
    public class Communicator
    {
        ...
        {
            ReceiveHandler =
                ReceiveCompleted; // Add receive delegate to delegate Handler
            SendHandler = SendCompleted; // Add send delegate to delegate Handler
            _sendArg = sendArg;
            ...
            CancelSendDequeue =
                new CancellationTokenSource(); // This will cancel send dequeuing
            ...
            void Send()
        {
            ...
            try
            {
                ...
                _sendPendingThread =
                    new Thread(() => SendCompleted(null, _sendArg));
        ...
        {
            if (error.LastOperation != SocketAsyncOperation.Receive || !OperationState) return;
         ...
         *  -> current position pointer stores the position of receiver buffer, and
         * sourceBuffer pointer is set to start position of new bytes received. and
         * continues reading from there position Read parameter still points the
         * desired position in the buffer
         * 2. Packet finished before data from source buffer was all read
         *  -> After packet finishes, packet buffer is processed, and reset.
         * current position buffer and position read buffer is again set to Header
         * size
         */
        ...
        public
            bool ProcessReceive()
        {
            // Return value is whether process succeeded
            ...
            try
            {
                ...
                while (sourceRemainBytes > 0 && OperationState)
                {
                        ...
                        PacketConsts.HeaderSize) // Header has not been completely read yet
                    {
                        ...
                        if (!completed || !OperationState) continue;
                        if (AnalyzeHeader())
                        {
                            // Header was valid
                            _receiverMembers.PositionToReadUntil =
            ...
            bool AnalyzeHeader()
        {
                ...
                PacketConsts.HeaderSign)
            {
                ...
                switch (_receiverMembers.DataPacket.Header[HeaderMemberStartIndex.EndPoint])
                {
                    case PacketConsts.MoreBytes:
                        _receiverMembers.DataPacket.MorePackets = true;
                        break;
                    case PacketConsts.LastByte:
